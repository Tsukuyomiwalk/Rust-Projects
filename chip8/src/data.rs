...
impl OpCode {
    ...
    pub fn extract_word(self, index: usize) -> Word {
        (self.index_in_memory(index, 8) & 0xFF) as Word
    }

    fn index_in_memory(self, index: usize, offset: usize) -> u16 {
        self.0 >> (index * offset)
    }
    pub fn extract_nibble(self, index: usize) -> Nibble {
        Nibble::try_from((self.index_in_memory(index, 4) & 0xF) as u8).unwrap()
    }
